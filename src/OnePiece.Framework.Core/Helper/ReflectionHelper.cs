...
namespace OnePiece.Framework.Core
{
    public class ReflectionHelper
    {
        ...
        public static List<KeyValuePair<string, string>> PropertyTraslation<T>(IEnumerable<string> excludedProperties, Type includeAttribute, Type excludedAttribute)
        {
            var includedAttributes = default(List<Type>);

            if (includeAttribute != null)
                includedAttributes = new List<Type> { includeAttribute };

            var excludedAttributes = default(List<Type>);
            if (excludedAttribute != null)
                excludedAttributes = new List<Type> { excludedAttribute };

            return PropertyTraslation<T>(includedAttributes, excludedAttributes, excludedProperties);
        }

        /// <summary>
        /// Get type's display name mapping.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="includedAttributes">Only give the result which has the specified attribute.</param>
        /// <param name="excludedAttributes">Only give the result which does not has the specified attribute.</param>
        /// <param name="excludedProperties">The property to exclude</param>
        /// <returns></returns>
        public static List<KeyValuePair<string, string>> PropertyTraslation<T>(IEnumerable<Type> includedAttributes = null, IEnumerable<Type> excludedAttributes = null, IEnumerable<string> excludedProperties = null)
        {
            var type = typeof(T);
            if (includedAttributes == null) includedAttributes = new List<Type>();
            if (excludedAttributes == null) excludedAttributes = new List<Type>();
            ...
            var properties = from p in type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                             ...
                             where attributes.Any(x => includedAttributes.Contains(x.GetType())) &&
                             !attributes.Any(x => excludedAttributes.Contains(x.GetType()))
                             && !excludedProperties.Contains(p.Name)
                             select p;

            if (!includedAttributes.Any())
            {
                properties = from p in type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                             let attributes = p.GetCustomAttributes(true)
                             where !attributes.Any(x => excludedAttributes.Contains(x.GetType()))
                             && !excludedProperties.Contains(p.Name)
                             select p;
            }

            var dictionary = properties.ToDictionary(x => x.Name, x =>
            {
                var displayAttribute = x.GetCustomAttributes(typeof(DisplayAttribute), true).FirstOrDefault() as DisplayAttribute;
        ...
        public static string Name<TModel, TValue>(Expression<Func<TModel, TValue>> propertySelector, char delimiter = '.', char endTrim = ')')
        {
        ...
        public static string GetDisplayName<TSource>(Expression<Func<TSource, object>> Field)
        {
            var name = (Field.Body as MemberExpression ?? ((UnaryExpression)Field.Body).Operand as MemberExpression).Member.Name;

            var property = typeof(TSource).GetProperty(name);
            if (property != null)
            {
                var attr = property.GetCustomAttributes(typeof(DisplayAttribute), true).FirstOrDefault() as DisplayAttribute;
