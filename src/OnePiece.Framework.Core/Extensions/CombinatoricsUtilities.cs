...
namespace OnePiece.Framework.Core.Extensions
{
    ...
    public static class CombinatoricsUtilities
    {
        ...
        public static void GetAllCombinations<T>(this IList<T> list, Action<IList<T>> action, Func<bool> stopAction = null, bool withRepetition = false)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (action == null)
            ...
            {
                if (stopAction != null && stopAction()) break;

                list.GetCombinations<T>(action, i, withRepetition);
            }
        }
