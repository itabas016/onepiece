...
namespace OnePiece.Framework.RedisMapper
{
    public class RedisService : IRedisService, IDisposable
    {
        ...
        public List<string> GetValuesFromHash(string hashId, List<string> keys)
        {
            using (var Redis = RedisClientManager.GetClient())
            {
                return Redis.GetValuesFromHash(hashId, keys.ToArray());
            }
        }
        ...
        {
            var redisServers = RedisConfigKeys.REDIS_READ_WRITE_SERVERS.ConfigValue().Split(';');
        ...
        public string IPDetail(long ipVal)
        {
            ...
            using (var redis = RedisClientManager.GetReadOnlyClient())
            {
                ...
                int start = 0;
                // redisClient ZCard Method return type is long
                // in the version 3.9.3.0 the type is int
                // so this way will throw exception
                int end = (int)lastIndex;
