...
namespace OnePiece.Framework.Tests.Core.Extensions
{
    public class StringExtensionsTest
    {
        ...
        [Fact]
        public void ToUInt32_Tests()
        {
            Assert.Equal((uint)1, "1".ToUInt32());
            Assert.Equal((uint)0, "".ToUInt32());
            Assert.Equal((uint)0, "0".ToUInt32());
            Assert.Equal((uint)0, "33a".ToUInt32());
            Assert.Equal((uint)0, "false".ToUInt32());
            Assert.Equal((uint)1, true.ToUInt32());
            Assert.Equal((uint)0, "1.0".ToUInt32());
            Assert.Equal((uint)111, "1.0".ToUInt32((uint)111));
            Assert.Equal((uint)0, "1.111".ToUInt32());
            Assert.Equal((uint)222, "1.111".ToUInt32(() => (uint)222));

            var d = default(string);
            Assert.Equal((uint)0, d.ToUInt32());
        }
        ...
        public void TakeLength()
        {
            ...
            Assert.Equal("0", str.TakeLength(-1));
            Assert.Equal("7890", str.TakeLength(-4));
            Assert.Equal("12345678901234567890", str.TakeLength(-114));
        }
        ...
        {
            var path = @"c:\a\b\c/d";
            var ret = path.SafePath();
            Assert.Equal(@"c:\a\b\c\d", ret);

            Assert.Equal(@"c:\a\b\c\d", @"c:\a\b\/c//d".SafePath());
            Assert.Equal(@"c:\a\b\c\d", @"c:\\\\a\/b\c\d".SafePath());
            Assert.Equal(@"c:/a/b/c/d", @"c:\a\\\\b\/c//d".SafePath(() => true));
        }
